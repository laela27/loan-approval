...
st.write("**c.1.1 Membuat Model dengan CIBIL Score**")
...
# Inisialisasi DummyClassifier dengan strategi most_frequent
...
# Prediksi probabilitas pada data pelatihan (untuk AUC-ROC)
y_train_pred_proba_baseline = baseline_model.predict_proba(X_train)[:, 1]

# Evaluasi baseline model pada data pelatihan
...
svc = SVC(probability=True, class_weight='balanced')  # Mengatur probability=True untuk menghitung AUC-ROC
svc.fit(X_train, y_train)

# Prediksi menggunakan model SVC pada data pelatihan
y_train_pred_svc = svc.predict(X_train)

# Prediksi probabilitas pada data pelatihan (untuk AUC-ROC)
y_train_pred_proba_svc = svc.predict_proba(X_train)[:, 1]

# Evaluasi model SVC pada data pelatihan
...
rf = RandomForestClassifier(class_weight='balanced', random_state=42)  # Mengatur class_weight='balanced'
rf.fit(X_train, y_train)

# Prediksi menggunakan model Random Forest pada data pelatihan
...
# Prediksi menggunakan model XGBoost pada data pelatihan
...
st.write("**c.1.3 Testing di Model Terbaik**")
...
st.write("**Evaluasi Model Random Forest pada Data Test**")
...
st.write("**c.1.5 Feature Importance**")
...
st.write("**Feature Importance dari Model Random Forest**")
...
st.write("**c.2.1 Membuat model Tanpa CIBIL Score**")
...
st.write("**c.2.2 Analisis dan Pemilihan Model Terbaik**")
...
st.write("**c.2.3 Testing di Model Terbaik**")
# Prediksi model pada test set
y_test_pred_rf = rf.predict(X_test)
...
# Visualisasi confusion matrix untuk Random Forest
st.write("**c.2.4 Confusion Matrix**")
plt.figure(figsize=(10, 7))
sns.heatmap(conf_matrix, annot=True, fmt="d", cmap="Blues", cbar=False)
...
st.write("**c.2.5 Feature Importance**")
